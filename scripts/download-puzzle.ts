...
async function fetchWithSession(
  ...
  if (!response.ok) {
    if (response.status === 404) {
      throw new Error(
        `Puzzle not found. Day ${url.match(/day\/(\d+)/)?.[1]} may not be available yet.`,
      )
    }
    if (response.status === 400) {
      throw new Error(
        'Invalid session cookie. Please check your AOC_SESSION environment variable.',
      )
    }
    throw new Error(
...
async function downloadPuzzle(
  ...
  // Extract all puzzle descriptions from the HTML
  // The puzzle parts are in <article class="day-desc"> tags (can be 1 or 2)
  ...
  // Process each article and combine them
  const processedArticles = articles.map((articleHtml, index) => {
    let markdown = articleHtml
  ...
  // Add day header
  const dayTitle =
    html.match(/<h2>--- Day \d+: (.*?) ---<\/h2>/)?.[1] || `Day ${day}`
...
async function saveFiles(
  ...
  // Write puzzle.md (will overwrite if it exists)
  const puzzleExists = existsSync(puzzlePath)
  await writeFile(puzzlePath, puzzle, 'utf-8')
  if (puzzleExists) {
    console.log(`Overwritten puzzle description at ${puzzlePath}`)
  } else {
    console.log(`Saved puzzle description to ${puzzlePath}`)
  }

  // Write input (will overwrite if it exists)
  const inputExists = existsSync(inputPath)
  await writeFile(inputPath, input, 'utf-8')
  if (inputExists) {
    console.log(`Overwritten input at ${inputPath}`)
  } else {
...
async function main() {
  ...
  if (!session) {
    ...
    console.error(
      'Get your session cookie from https://adventofcode.com (browser dev tools -> Application -> Cookies)',
    )
    process.exit(1)
  }
